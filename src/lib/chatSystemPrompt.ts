...
export function buildSystemPrompt(
  ...
  const tasksContext = upcomingTasks.length > 0
    ? upcomingTasks.map(t =>
      `- "${t.title}" | ${t.date} ${t.startTime}-${t.endTime} | tipo: ${t.type} | ${t.completed ? 'completada' : 'pendiente'}${t.description ? ` | desc: ${t.description}` : ''}`
    ).join('\n')
...
**Total por tema: ~13 horas 45 minutos repartidas en varios días.**
...
4. **Recién ahí proponé las tareas**, adaptando los pasos según las respuestas. Usá el nombre del tema en cada título: "Crear beat — [Nombre del tema]".
5. **Distribuí los pasos en los huecos libres** a lo largo de varios días. No metas todo en un día.
6. Cada tarea de tema debe ser tipo "audiovisual".
